...
using JETMASHRADIO.TextureModder;
using System.Drawing.Imaging;
using JETMASHRADIO;

namespace JETMASHRADIO
{
    public partial class TextureModderForm : Form
    {
        ...
        private void convertToPVRButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var pvrData = JETMASHRADIO.JSR.BinToPVR(binData);
                JETMASHRADIO.JSR.SavePVRFiles(pvrData, ExportedTexturesDirectoryTextBox.Text);
            }
            catch (Exception ex)
            {
        ...
        private void convertToBinFileButton_Click(object sender, EventArgs e)
        {
            ...
            {
                if (Mode == TextureModderMode.Character)
                    System.IO.File.WriteAllBytes(ExportedBinOutputDirectoryTextBox.Text + "/" + outFile, JETMASHRADIO.JSR.PVRToBin(character, pvrData));
                else if (Mode == TextureModderMode.Stage)
                    System.IO.File.WriteAllBytes(ExportedBinOutputDirectoryTextBox.Text + "/" + outFile, JETMASHRADIO.JSR.PVRToTXP(stage, pvrData));
            }
            catch (Exception ex)
            {
                ImportErrorLabel.Text = ex.Message;
        ...
        private void convertToPNGButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                JETMASHRADIO.JSR.SavePVRAsPNGFiles(config, pvrData, ExportedTexturesDirectoryTextBox.Text);
